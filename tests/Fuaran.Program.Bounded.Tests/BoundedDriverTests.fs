module Fuaran.Program.Bounded.Tests.BoundedDriverTests

// ─── The no-`'Msg` bounded driver (server placement) ────────────────
//
// Drives an emitted, wire-decoded tree with NO hand-authored update / view.
    ...
    testList
        ...
        [ test "init re-resolves a State-bound TextSource to its default Literal" {
          ...
          test "dual-host parity: resolveTree is deterministic and matches direct resolution" {
              ...
              let r1 = Resolve.resolveTree store tree
              let r2 = Resolve.resolveTree store tree
